...
export async function checkIsAdmin(userId) {
  ...
  try {
    ...
    const adminDocRef = doc(getDb(), ADMIN_USERS_COLLECTION, userId);
    ...
    for (const collectionName of ALTERNATIVE_ADMIN_COLLECTIONS) {
      try {
        const altAdminDocRef = doc(getDb(), collectionName, userId);
...
// ============================================
// PROFILE PAGINATION (Firestore)
// ============================================

/**
 * Get paginated profiles from Firestore (admin view)
 * @param {import('firebase/firestore').QueryDocumentSnapshot | null} lastDoc - Cursor for pagination
 * @param {number} pageSize - Number of profiles per page
 * @returns {Promise<{ profiles: Object[], lastDoc: import('firebase/firestore').QueryDocumentSnapshot | null }>}
 */
export async function getAdminProfiles(lastDoc, pageSize = 20) {
  try {
    const constraints = [orderBy('createdAt', 'desc'), limit(pageSize)];
    if (lastDoc) {
      constraints.push(startAfter(lastDoc));
    }
    const q = query(collection(getDb(), 'profiles'), ...constraints);
...
/**
 * Get total profile count using server-side aggregation (zero documents downloaded)
 * @returns {Promise<number>}
 */
export async function getProfileCount() {
  try {
    const q = query(collection(getDb(), 'profiles'));
 ...
 * @returns {Promise<{ profiles: Object[], lastDoc: import('firebase/firestore').QueryDocumentSnapshot | null }>}
 */
export async function searchAdminProfiles(searchTerm, pageSize = 20) {
  try {
...
export async function getPromoterCodes() {
  try {
    const q = query(collection(getDb(), 'promoterCodes'));
    const snapshot = await getDocs(q);
