...
import { filterNotDeleted } from './softDelete';
...
export async function getPublicPosts(lastDoc = null, pageSize = PAGE_SIZE) {
  ...
  // Filter out soft-deleted posts (client-side for backward compatibility)
  const activePosts = filterNotDeleted(posts);
  const newLastDoc = snapshot.docs[snapshot.docs.length - 1] || null;

  return { posts: activePosts, lastDoc: newLastDoc };
}
 ...
 * @param {string|null} [params.contentWarning] - Optional content warning message
