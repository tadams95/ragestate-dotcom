...
let _db;
const getDb = () => (_db ??= getFirestore());

const PURCHASES_COLLECTION = 'purchases';
const CUSTOMERS_COLLECTION = 'customers';
const PAGE_SIZE = 20;

function toSafeDate(primaryValue, fallbackValue = null, defaultValue = null) {
  ...
  return parseValue(primaryValue) || parseValue(fallbackValue) || defaultValue;
}
...
export async function getPurchase(purchaseId) {
  ...
  const purchaseDoc = await getDoc(doc(getDb(), PURCHASES_COLLECTION, purchaseId));
  ...
  return {
    id: purchaseDoc.id,
    ...data,
    orderDate: toSafeDate(data.orderDate, data.dateTime, new Date()),
  };
}
...
export async function getUserPurchases(userId, lastDoc = null, pageSize = PAGE_SIZE) {
  ...
  const userPurchasesRef = collection(getDb(), CUSTOMERS_COLLECTION, userId, 'purchases');
  const constraints = [orderBy('orderDate', 'desc'), limit(pageSize)];
...
/**
 * Get total purchase count using server-side aggregation
 * @returns {Promise<number>}
 */
export async function getPurchaseCount() {
  try {
    const q = query(collection(getDb(), PURCHASES_COLLECTION));
...
export async function getAllPurchases(lastDoc = null, pageSize = PAGE_SIZE) {
  ...
  const q = query(collection(getDb(), PURCHASES_COLLECTION), ...constraints);
  const snapshot = await getDocs(q);
...
export async function getPurchaseDetails(purchaseId) {
  ...
  const purchaseDocRef = doc(getDb(), PURCHASES_COLLECTION, purchaseId);
  ...
  return {
    id: purchaseId,
    ...purchaseData,
    orderDate: toSafeDate(purchaseData.orderDate, purchaseData.dateTime, new Date()),
    tickets,
    physicalItems,
    digitalItems,
    raw: purchaseData,
...
export async function savePurchase(purchaseData) {
  ...
  const purchaseRef = doc(collection(getDb(), PURCHASES_COLLECTION));
...
export async function searchPurchasesByEmail(email, limitCount = 10) {
  ...
  const q = query(
    collection(getDb(), PURCHASES_COLLECTION),
  ...
  return snapshot.docs.map((docSnap) => {
    ...
    return {
      id: docSnap.id,
      ...data,
      orderDate: toSafeDate(data.orderDate, null, new Date()),
    };
  });
}
