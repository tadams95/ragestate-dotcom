...
import { doc, getDoc, getDocs, query, setDoc, updateDoc, where, collection, getFirestore } from 'firebase/firestore';
...
export async function getUser(userId) {
  ...
  const userDoc = await getDoc(doc(getDb(), USERS_COLLECTION, userId));
...
export async function getProfile(userId) {
  ...
  const profileDoc = await getDoc(doc(getDb(), PROFILES_COLLECTION, userId));
...
export async function getProfileByUsername(username) {
  ...
  const usernameDoc = await getDoc(doc(getDb(), USERNAMES_COLLECTION, usernameLower));
...
export async function updateProfile(userId, updates) {
  ...
  const profileRef = doc(getDb(), PROFILES_COLLECTION, userId);
...
export async function getCustomer(userId) {
  ...
  const customerDoc = await getDoc(doc(getDb(), CUSTOMERS_COLLECTION, userId));
...
export async function getUserDisplayInfo(userId) {
  const [customerDoc, profileDoc] = await Promise.all([
    getDoc(doc(getDb(), CUSTOMERS_COLLECTION, userId)),
    getDoc(doc(getDb(), PROFILES_COLLECTION, userId)),
  ]);
