      ...
      return snapshot.docs.map((doc) => {
        ...
        return {
          ...
          shippingAddress: data.addressDetails?.address
            ? `${data.addressDetails.address.line1}, ${data.addressDetails.address.city}, ${data.addressDetails.address.state} ${data.addressDetails.address.postal_code || data.addressDetails.address.postalCode || ''}`
            : 'No shipping info',
        };
      });
    } catch (error) {
  ...
  const fetchPurchaseDetails = async (purchaseId) => {
    try {
      ...
      return {
        ...
        id: purchaseId,
        orderDate:
          purchaseData.orderDate?.toDate() || purchaseData.dateTime?.toDate() || new Date(),
        tickets,
        physicalItems,
        digitalItems,
