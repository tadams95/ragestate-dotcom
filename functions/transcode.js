 ...
 */
async function updatePostProcessingStatus(postId, isProcessing, optimizedUrl = null) {
  const postRef = db.collection('posts').doc(postId);

  const update = { isProcessing };
  if (optimizedUrl) {
    // Add optimized URL to the array (or create it)
    update.optimizedMediaUrls = admin.firestore.FieldValue.arrayUnion(optimizedUrl);
  }
...
exports.onVideoUpload = onObjectFinalized(
  ...
  async (event) => {
    ...
    if (!filePath.startsWith('posts/') || filePath.startsWith('posts-optimized/')) {
    ...
    const tempOutputPath = path.join(os.tmpdir(), `output_${Date.now()}_${outputFileName}`);
