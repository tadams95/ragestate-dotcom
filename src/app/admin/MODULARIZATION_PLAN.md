...
2.  **Extract Tab Content:**
    ...
    - Consider extracting common UI elements like `loadingState` and `errorState` into shared components if needed.
...
4.  **Extract Helper Functions:**
    ...
    - Move utility functions (`formatDate`, `formatCurrency`, `getStatusColor`) to a separate utility file (e.g., `src/utils/formatters.js` or `src/utils/helpers.js`).
    - Import these functions into `AdminPage` and any new components that require them.

5.  **Refactor `AdminPage` (`src/app/admin/page.js`):**
    - Remove the large inline JSX blocks for tabs and the modal.
    - Remove the helper function definitions (`formatDate`, `formatCurrency`, `getStatusColor`).
    - Import the newly created components (`DashboardTab`, `OrdersTab`, `UsersTab`, `SettingsTab`, `OrderDetailsModal`) and the helper functions from the utility file.
    - Update the `tabComponents` object to render the imported tab components, passing down the required props.
    - Render the `OrderDetailsModal` component conditionally, passing its required props.
    - Retain the primary state management (`useState` hooks) and data fetching logic (`useEffect`) within `AdminPage`, making it function as a container component.
```
