...
export async function POST(req) {
  try {
    ...
    for (const base of bases) {
      const endpoint = base.replace(/\/$/, '') + '/scan-ticket/preview';
      try {
    ...
    console.error('scan-ticket/preview proxy: all bases failed', lastError?.message);
    return NextResponse.json({ error: 'Service unavailable' }, { status: 503 });
  } catch (e) {
    console.error('scan-ticket/preview proxy error:', e);
