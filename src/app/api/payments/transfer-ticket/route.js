...
export async function POST(request) {
  try {
    ...
    if (!payload.ragerId || !payload.eventId || !payload.recipientEmail || !payload.senderUserId) {
    ...
    for (const base of bases) {
      const endpoint = base.replace(/\/$/, '') + '/transfer-ticket';
      try {
    ...
    // All attempts failed
    return NextResponse.json(
      { error: lastError?.message || 'Failed to transfer ticket' },
      { status: lastError?.status || 502 },
    );
  } catch (err) {
    console.error('transfer-ticket proxy error:', err);
