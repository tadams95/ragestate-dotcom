...
// Prefer Cloud Run URL for 2nd-gen Functions to avoid redirects
function getFunctionBaseUrl() {
  // 1) Explicit env (preferred)
  const explicit = process.env.STRIPE_FN_URL || process.env.NEXT_PUBLIC_STRIPE_FN_URL;
  if (explicit) return explicit;

  // 2) Derive projectId from common env
  const projectId =
  ...
  // Use cloudfunctions.net URL (Next server can follow redirects to Cloud Run)
...
const FN_ENDPOINT = FN_BASE ? FN_BASE.replace(/\/$/, '') + '/create-payment-intent' : '';
