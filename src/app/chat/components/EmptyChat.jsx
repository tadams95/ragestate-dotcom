'use client';

import { memo } from 'react';

/**
 * @typedef {Object} EmptyChatProps
 * @property {() => void} [onNewChat] - Callback when "Start a Chat" is clicked
 */

/**
 * Empty state component for when there are no chats
 * @param {EmptyChatProps} props
 */
function EmptyChat({ onNewChat }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center p-8">
      {/* Icon */}
      <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--bg-elev-2)]">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="h-8 w-8 text-[var(--text-tertiary)]"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z"
          />
        </svg>
      </div>

      {/* Text */}
      <h2 className="mb-2 text-xl font-semibold text-[var(--text-primary)]">No messages yet</h2>
      <p className="mb-6 text-center text-[var(--text-secondary)]">
        Start a conversation or join an event chat
      </p>

      {/* CTA Button */}
      {onNewChat && (
        <button
          onClick={onNewChat}
          className="rounded-lg bg-[var(--accent)] px-6 py-3 font-semibold text-white transition-opacity hover:opacity-90"
        >
          Start a Chat
        </button>
      )}
    </div>
  );
}

export default memo(EmptyChat);
