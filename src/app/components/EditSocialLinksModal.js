...
/**
 * @typedef {Object} EditSocialLinksModalProps
 * @property {boolean} isOpen
 * @property {() => void} onClose
 * @property {string} userId
 * @property {object|null} initialSocialLinks
 * @property {(cleaned: object) => void} onSave
 */

/**
 * Modal for editing social media links from the profile page
 * @param {EditSocialLinksModalProps} props
 */
function EditSocialLinksModal({ isOpen, onClose, userId, initialSocialLinks, onSave }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSocialLinks({ ...empty, ...(initialSocialLinks || {}) });
      setSocialLinkErrors({});
      setSaving(false);
    }
  }, [isOpen, initialSocialLinks]);
  ...
  const handleChange = useCallback((platform, value) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'profiles', userId), { socialLinks: cleaned }, { merge: true });
  ...
  }, [saving, userId, socialLinks, onSave, onClose]);
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-md rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-elev-1)] p-6 shadow-xl">
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">
            Add your social media profiles so others can find you.
          </p>
          ...
          <div className="mt-5 space-y-4">
            {SOCIAL_PLATFORM_ORDER.map((platform) => {
              ...
              const error = socialLinkErrors[platform];
              ...
              return (
                <div key={platform}>
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border bg-[var(--bg-elev-2)] px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] outline-none transition-colors focus:ring-1 ${
                      error
...
export default memo(EditSocialLinksModal);
