 ...
 * @property {string} slug - Event slug / document ID
 * @property {Object|null} initialEvent - Server-fetched event data (serialized)
 */

/**
 * Client component for event detail page.
 * Handles admin controls, email capture, and interactive features.
 * @param {EventDetailClientProps} props
 */
export default function EventDetailClient({ slug, initialEvent }) {
  ...
  // Client-side fetch for draft events or when no initialEvent provided
  useEffect(() => {
  ...
  }, [slug, db, currentUser, initialEvent]);

  // Admin check for server-provided events
  useEffect(() => {
    if (!initialEvent || !currentUser) return;
    let cancelled = false;
              ...
              <div className="mb-8 flex flex-col items-center text-center">
                ...
                <p className="mt-2 text-[var(--text-secondary)]">
                  {(() => {
                    ...
                    const dt = e?.dateTime || e?.startDate;
                    let d = null;
                    try {
                      d =
